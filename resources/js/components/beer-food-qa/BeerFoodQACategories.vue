<template>
    <div class="col-12">
        ...
        <create-and-edit-beer-food-q-a-category :beer="beers"/>
    </div>
</template>
...
import CreateAndEditBeerFoodQACategory from "./CreateAndEditBeerFoodQACategory";

export default {
    name: "BeerFoodQACategories",
    components: {
        Spinner,
        CreateAndEditBeerFoodQACategory
    },
    data() {
        return {
            pageIsLoading: true,
    ...
    methods: {
        loadBeerQACategories(page = 1) {
            axios.get('/admin/beer-food-qa-categories')
                .then(response => {
        ...
        deleteBeerQACategory(id) {
            Swal.fire({
                ...
                .then((result) => {
                    if (result.value) {
                        axios.delete(`/admin/beer-food-qa-categories/${id}`)
                            .then(response => {
                                if (response.data[0] === 'success') {
        ...
        loadSearchedBeerQACategories(page = 1) {
            axios.get(`/admin/beer-food-qa-categories/${this.searchKeyword}/search?page=${page}`)
                .then(response => {
                    if (response.data[0] === "success") {
                        this.beersQaCategoriesPagination = response.data[1];
                        this.beerQaCategories = response.data[1].data;
        ...
        searchBeerQACategories() {
            if (!(this.searchKeyword === '')) {
                ...
                axios.get(`/admin/beer-food-qa-categories/${this.searchKeyword}/search`)
                    .then(response => {
                        if (response.data[0] === "success") {
                            this.beersQaCategoriesPagination = response.data[1];
                            this.beerQaCategories = response.data[1].data;
    ...
    mounted() {
        this.loadBeerQACategories();
        this.$emit('loadBreadcrumbLink', {url: '/beer-food-qa-categories', pageName: 'Pivo i hrana - QA kategorije'});
        EventBus.$on('load-beer-food-qa-categories', () => this.loadBeerQACategories());
    }
}
