...
export default {
    name: "CreateAndEditBeerFoodCategory",
    components: {
        ImageUploader
    },
    data() {
    ...
    methods: {
        ...
        storeCategory() {
            ...
            axios.post(`/admin/beer-food-categories/store`, form, config)
                .then(response => {
        ...
        updateCategory() {
            ...
            const config = {headers: {'content-type': 'multipart/form-data'}};
            axios.post(`/admin/beer-food-categories/${this.categoryForm.id}/update`, form, config)
                .then(response => {
                    this.storeUpdateDisabled = false;
                    if (response.data[0] === 'success') {
                        $('#create-and-edit-modal').modal('hide');
