...
export default {
    name: "CreateAndEditBeerQaCategory",
    components: {
        ImageUploader
    },
    data() {
    ...
    methods: {
        ...
        storeBeer() {
            ...
            axios.post(`/admin/beer-qa-categories/store`, form, config)
                .then(response => {
        ...
        updateBeer() {
            ...
            const config = {headers: {'content-type': 'multipart/form-data'}};
            axios.post(`/admin/beer-qa-categories/${this.beerForm.id}/update`, form, config)
                .then(response => {
                    this.storeUpdateDisabled = false;
                    if (response.data[0] === 'success') {
                        $('#create-and-edit-modal').modal('hide');
